...
public class DayroomService {
    ...
    public String getFormattedLogTimesById(int id) {
        ...
        for (int i = 0; i < size; i += 2) {
            sb.append(logTimesById.get(i) + "-" + logTimesById.get(i + 1) + "\n");
        }
        if (isOdd(logTimesById.size())) {
            sb.append(logTimesById.get(size) + "-\n");
        }
        return sb.toString();
    }
    ...
    public String getTotalDuration(int id) {
        var logTimesById = getLogTimesById(id);
        if (isOdd(logTimesById.size())) {
            logTimesById.add(new LogTime(15, 0));
        }
        int minute = 0;
        for (int i = 0; i < logTimesById.size(); i += 2) {
            minute += logTimesById.get(i).getDuration(logTimesById.get(i + 1));
        }
        logTimesById = getLogTimesById(id);
